...
def test_start_writes_to_runs_jsonl(tracker, tmp_path):
...
def test_finish_appends_success_record(tracker, tmp_path):
    run_dir = tracker.start("pretext_to_asm", "RC-1234", "sDipInt39")
...
def test_finish_appends_failed_record(tracker, tmp_path):
    run_dir = tracker.start("pretext_to_asm", "RC-1234", "sDipInt39")
...
def test_pending_jobs_returns_started_with_job_id(tracker):
    ...
    r2 = tracker.start("sex_matcher", "RC-1234", "sDipInt39")
...
def test_print_only_does_not_create_files(tmp_path):
    tracker = RunTracker(tmp_path, print_only=True)
    run_dir = tracker.start("pretext_to_asm", "RC-1234", "sDipInt39")
    assert not run_dir.exists()
    assert not (tmp_path / ".grit" / "runs.jsonl").exists()


def test_verify_outputs_ok(tracker, tmp_path):
    run_dir = tracker.start("pretext_to_asm", "RC-1234", "sDipInt39")
...
def test_verify_outputs_missing(tracker, tmp_path):
    run_dir = tracker.start("pretext_to_asm", "RC-1234", "sDipInt39")
    # No files created
...
def test_verify_outputs_setup_curation_checks_workdir(tmp_path):
